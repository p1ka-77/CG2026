...
// Geometry Pass дл€ tessellated surface.
// ѕишет данные в GBuffer: position / normal / albedo+roughness.

// t0 Ч diffuse / albedo texture
// t1 Ч normal map
// t2 Ч displacement / height map
...
// “акой же Object CB, как в GeometryPass.hlsl
...
// “акой же Pass CB, как в GeometryPass.hlsl
...
// ќтдельные настройки тессел€ции
...
// –асчЄт tessellation factor в зависимости от рассто€ни€ до камеры.
float CalcTessFactor(float3 worldPos)
{
    float distToEye = distance(worldPos, gEyePosW);

    // Ѕлизко к камере Ч tessellation высока€.
    // ƒалеко от камеры Ч tessellation низка€.
    float s = saturate(
...
// Patch constant function дл€ quad patch из 4 control points.
...
DomainOut DS(PatchTess patchTess, float2 uv : SV_DomainLocation, const OutputPatch<HullOut, 4> patch)
{
    ...
    // Height/displacement.
    // ЅерЄм значение из displacement map и двигаем вершину вдоль нормали.
...
// јнимированна€ волна.
// »спользуем локальные координаты пола и врем€ из cbPass.
...
// »тоговое смещение: карта высот + волна.
...
// ¬осстановление normal map через экранные производные.
// Ёто не требует tangent в вершинах.
